...
class SinglyLinkedList(object):
    ...
    def insert_after(self, node, value):
        """在链表的某个指定Node节点之后插入一个存储value值的Node节点
        :param node: 指定的一个node节点
        :param value: 将要存储在新Node节点的数据
        :return: void
        """
        if (node is None) or self.__head is None:  # 如果在一个空节点之后插入数据节点，则什么都不做
            return
